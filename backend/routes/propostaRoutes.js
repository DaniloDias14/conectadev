...
router.post("/", async (req, res) => {
  try {
    ...
    const propostaExistente = await pool.query("SELECT id FROM propostas WHERE desafio_id = $1 AND usuario_id = $2", [
      desafio_id,
      usuarioId,
    ])

    if (propostaExistente.rows.length > 0) {
      return res.status(409).json({ mensagem: "Você já enviou uma proposta para este desafio" })
    }
    ...
    const resultado = await pool.query(
      `INSERT INTO propostas (desafio_id, usuario_id, valor, justificativa, prazo_estimado) 
       VALUES ($1, $2, $3, $4, $5) 
       RETURNING *`,
      [desafio_id, valor, justificativa, prazo_estimado, usuarioId],
    )
    ...
    console.error(erro)
    res.status(500).json({ mensagem: "Erro ao enviar proposta" })
  }
})
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const propostaResultado = await pool.query("SELECT usuario_id, atualizado_em FROM propostas WHERE id = $1", [id])
    ...
    if (proposta.usuario_id !== usuarioId) {
      return res.status(403).json({ mensagem: "Sem permissão para editar esta proposta" })
    }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const propostaResultado = await pool.query("SELECT usuario_id FROM propostas WHERE id = $1", [id])
    ...
    if (propostaResultado.rows[0].usuario_id !== usuarioId) {
      return res.status(403).json({ mensagem: "Sem permissão para deletar esta proposta" })
    }
